...
def loading_screen(progress):
    ...
    pygame.draw.rect(screen, (0, 255, 0), (bar_x + 1, bar_y + 1, fill_width, bar_height - 2))
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == WINDOW and dragging_item is None:
                ...
                if not inventory_open:
                    ...
                    crafting_base = [None]*4
                    crafting_base_count = [0]*4
        if event.type == pygame.MOUSEBUTTONDOWN and not inventory_open: 
            if event.button == 3:
                block = get_block_at()
                if block and block.type == "crafting_table":
                    crafting_open = True
        if event.type == pygame.MOUSEBUTTONDOWN and (inventory_open or crafting_open):
            if event.button == 1:
                    ...
                    for i in range(9):
                        ...
                        if slot_rect.collidepoint(mouse_x, mouse_y):
                            if len(crafting_base) == 9 and crafting_base[i] is not None and crafting_base_count[i] > 1:
                                ...
                                temp_item = (crafting_base[i], split_count)
                            break
        if event.type == pygame.MOUSEBUTTONUP and dragging_item is not None:
            if event.button == 1:
                mouse_x, mouse_y = pygame.mouse.get_pos()
                ...
                if not returned:
                    ...
                    dragging_item = None
                    original_slot = None
        if (event.type == pygame.MOUSEBUTTONDOWN or event.type == pygame.MOUSEBUTTONUP) and (inventory_open or crafting_open):
    ...
    if in_block:
        ...
        if keys[SNEAK] and (keys[LEFT] or keys[RIGHT]) and not keys[JUMP]:
            ...
            if t is not None:
                if t in blocks:
                    if not pygame.Rect(block_x, block_y, BLOCK_SIZE, BLOCK_SIZE).colliderect(player_rect):
                        ...
                        if not "liquid" in blocks[t] and not keys[BACK_BUILD]:
